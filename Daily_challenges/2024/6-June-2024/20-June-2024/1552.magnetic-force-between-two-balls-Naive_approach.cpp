...
/*
    Naive approach: TLE
    Time complexity: O(n * (max_position-min_position))
...
class Solution {
public:
    bool can_we_place_balls(int dis,std::vector<int>& position,int m){
        int n=position.size();
    ...
    int maxDistance(vector<int>& position, int m){
        ...
        int distances=max_distance-min_distance;

        std::sort(position.begin(),position.end());

        for(int dis=distances;dis>=1;--dis){
            if(can_we_place_balls(dis,position,m)) return dis;
        }
        return 0;
    }
};
