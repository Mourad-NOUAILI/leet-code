    ...
    public:
        long long continuousSubarrays(vector<int>& nums){
            ...
            for(int r=0;r<n;++r){
                ...
                while(l<r && max_deq.front().first-min_deq.front().first>2){
                    // Shrink the window
                    l++;
                    // Remove indices that are now outside window
                    while(!max_deq.empty() && max_deq.front().second<l) max_deq.pop_front();
                    while(!min_deq.empty() && min_deq.front().second<l) min_deq.pop_front();
